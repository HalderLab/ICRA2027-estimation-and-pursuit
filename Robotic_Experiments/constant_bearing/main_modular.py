...
class TwoAgentModularNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("follower_cmd_topic", "/burger3/cmd_vel")
        self.declare_parameter("lidar_objects_topic", "/burger3/lod_objects")
        ...
        self.declare_parameter("target_mode", "physical_lock")
        ...
        self.declare_parameter("rho_bias", 0.0475)
        self.declare_parameter("alpha2_bias", 0.04)
        ...
        self.declare_parameter("follower_rigid_body_name", "19")
        ...
        # Leader-input switch:
        #   "constant"     -> use leader_v and leader_u above
        #   "time_varying" -> use the sinusoidal/cosinusoidal functions below
        self.declare_parameter("leader_input_mode", "time_varying")

        # Time-varying truth sent ONLY to the physical leader robot.
        #
        #   value(t) = offset + amplitude * wave(omega*t + phase)
        #
        # waveform must be "sin" or "cos"; omega is in rad/s.
        self.declare_parameter("leader_v_tv_offset", 0.10)
        self.declare_parameter("leader_v_tv_amplitude", 0.03)
        self.declare_parameter("leader_v_tv_omega", 0.3)
        self.declare_parameter("leader_v_tv_phase", 0.0)
        self.declare_parameter("leader_v_tv_waveform", "sin")

        self.declare_parameter("leader_u_tv_offset", 0.13)
        self.declare_parameter("leader_u_tv_amplitude", 0.05)
        self.declare_parameter("leader_u_tv_omega", 0.3)
        self.declare_parameter("leader_u_tv_phase", 0.0)
        self.declare_parameter("leader_u_tv_waveform", "cos")

        self.declare_parameter("follower_v", 0.11)
        self.declare_parameter("mu", 2.0)
        self.declare_parameter("phi", math.radians(20.0))
        ...
        # ==============================================================
        self.declare_parameter("alpha1_initial_guess", 0.0)
        self.declare_parameter("v1_initial_guess", 0.22)
        ...
        # These two remain the CONSTANT/NOMINAL values used to initialize
        # the EKF/PF model.  In time_varying mode, the instantaneous physical
        # truth is deliberately NOT fed into EKF/PF.
        self.leader_v = float(self.get_parameter("leader_v").value)
        self.leader_u = float(self.get_parameter("leader_u").value)

        self.leader_input_mode = str(
            self.get_parameter("leader_input_mode").value
        ).strip().lower()
        if self.leader_input_mode not in {"constant", "time_varying"}:
            raise ValueError(
                "leader_input_mode must be 'constant' or 'time_varying'; got "
                f"{self.leader_input_mode!r}"
            )

        self.leader_v_tv_offset = float(
            self.get_parameter("leader_v_tv_offset").value
        )
        self.leader_v_tv_amplitude = float(
            self.get_parameter("leader_v_tv_amplitude").value
        )
        self.leader_v_tv_omega = float(
            self.get_parameter("leader_v_tv_omega").value
        )
        self.leader_v_tv_phase = float(
            self.get_parameter("leader_v_tv_phase").value
        )
        self.leader_v_tv_waveform = str(
            self.get_parameter("leader_v_tv_waveform").value
        ).strip().lower()

        self.leader_u_tv_offset = float(
            self.get_parameter("leader_u_tv_offset").value
        )
        self.leader_u_tv_amplitude = float(
            self.get_parameter("leader_u_tv_amplitude").value
        )
        self.leader_u_tv_omega = float(
            self.get_parameter("leader_u_tv_omega").value
        )
        self.leader_u_tv_phase = float(
            self.get_parameter("leader_u_tv_phase").value
        )
        self.leader_u_tv_waveform = str(
            self.get_parameter("leader_u_tv_waveform").value
        ).strip().lower()

        for name, waveform in (
            ("leader_v_tv_waveform", self.leader_v_tv_waveform),
            ("leader_u_tv_waveform", self.leader_u_tv_waveform),
        ):
            if waveform not in {"sin", "cos"}:
                raise ValueError(
                    f"{name} must be 'sin' or 'cos'; got {waveform!r}"
                )
        ...
        if self.leader_input_mode == "constant":
            self.get_logger().info(
                "Leader input mode: CONSTANT: "
                f"v1={self.leader_v:.3f} m/s, u1={self.leader_u:.3f} rad/s"
            )
        else:
            self.get_logger().info(
                "Leader input mode: TIME_VARYING: "
                f"v1(t)={self.leader_v_tv_offset:.3f}"
                f"+{self.leader_v_tv_amplitude:.3f}*"
                f"{self.leader_v_tv_waveform}("
                f"{self.leader_v_tv_omega:.3f}t+"
                f"{self.leader_v_tv_phase:.3f}); "
                f"u1(t)={self.leader_u_tv_offset:.3f}"
                f"+{self.leader_u_tv_amplitude:.3f}*"
                f"{self.leader_u_tv_waveform}("
                f"{self.leader_u_tv_omega:.3f}t+"
                f"{self.leader_u_tv_phase:.3f})"
            )
        self.get_logger().info(
    ...
    def _wave_value(
        ...
        arg = omega * t + phase
        if waveform == "sin":
            return offset + amplitude * math.sin(arg)
        return offset + amplitude * math.cos(arg)

    def leader_truth_command(self, now: float) -> tuple[float, float]:
        """Return the actual v1(t), u1(t) sent to Burger1.

        Time is measured from the synchronized release, so t=0 exactly when
        both robots receive their first nonzero command.

        IMPORTANT:
        In time_varying mode these instantaneous truth values are NOT passed
        into EKF/PF.  Their internal models continue using the nominal
        leader_v / leader_u values supplied at construction.
        """
        if self.leader_input_mode == "constant" or self.start_time is None:
            return self.leader_v, self.leader_u

        elapsed = max(0.0, float(now) - float(self.start_time))

        v1 = self._wave_value(
            elapsed,
            self.leader_v_tv_offset,
            self.leader_v_tv_amplitude,
            self.leader_v_tv_omega,
            self.leader_v_tv_phase,
            self.leader_v_tv_waveform,
        )
        u1 = self._wave_value(
            elapsed,
            self.leader_u_tv_offset,
            self.leader_u_tv_amplitude,
            self.leader_u_tv_omega,
            self.leader_u_tv_phase,
            self.leader_u_tv_waveform,
        )
        return v1, u1

    def bootstrap_motion_command(self, now: float) -> ControlCommand:
        """First command after synchronized release."""
        leader_v_cmd, leader_u_cmd = self.leader_truth_command(now)
        ...
        return ControlCommand(
            leader_v=leader_v_cmd,
            leader_u=leader_u_cmd,
            follower_v=self.follower_v,
            follower_u=follower_u,
        )

    def warmup_command(self, now: float) -> ControlCommand:
        leader_v_cmd, leader_u_cmd = self.leader_truth_command(now)
        return ControlCommand(
            leader_v=leader_v_cmd,
            leader_u=leader_u_cmd,
    ...
    def closed_loop_command(self, estimate, now: float) -> ControlCommand:
        leader_v_cmd, leader_u_cmd = self.leader_truth_command(now)
    ...
    def release_synchronized_start(self, now: float) -> None:
        ...
        command = self.bootstrap_motion_command(now)
    ...
    def on_objects(self, msg: ObjectsArray) -> None:
        ...
        if estimate.ready:
            log_command = self.closed_loop_command(estimate, receive_now)
        else:
    ...
    def on_timer(self) -> None:
        ...
        if (
            ...
            if self.mhe_warmup_move:
                command = self.warmup_command(now)
                self.cmd_vel.publish(command)
                self.current_command = command
            else:
                self.cmd_vel.stop()
        ...
        command = self.closed_loop_command(predicted, now)
        self.cmd_vel.publish(command)
        self.current_command = command
